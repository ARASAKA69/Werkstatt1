...
function saveNachbestellungKommentarUndRegal_(stockId, text, regal, expectedKommentar, expectedRegal, includeClosed) {
  ...
  function searchByOrderNumber(query) {
    try {
      ...
      return { found: false, message: "Sendung/Bestellnummer '" + shown + "' weder im Sheet, Gmail Lookup, Nachbestellung noch in Belegen gefunden." };
    } catch (err) {
...
function getRefurbishmentCachePayload() {
  ...
  function packzettelRowLight_(row, rowNumber) {
    var isN4 = pzIsN4pRow_(row);
    var refStock = isN4 ? pzN4pReferenzStock_(row) : "";
    var kennzeichen = String(row[6] || "").trim();
    var shownStock = isN4 ? (refStock || String(row[5] || "").trim()) : String(row[5] || "").trim();
    return {
      row: rowNumber,
  ...
  function packzettelRowMatchesQuery_(row, query) {
    ...
    var haystack = [
      row[3], row[4], row[5], row[6], row[8], row[1], row[7], row[12], row[13]
...
function pzFindAllSupplierHeaders_(text) {
  var patterns = [
...
function pzParseN4pPartsFromText_(raw) {
  var text = String(raw || "").replace(/\r/g, "\n");
  if (!text.trim()) return [];
  var headers = pzFindAllSupplierHeaders_(text);
  if (headers.length) {
    var all = [];
    for (var h = 0; h < headers.length; h++) {
      var from = headers[h].index;
      var to = h + 1 < headers.length ? headers[h + 1].index : text.length;
      var segment = text.slice(from, to);
      var segParts = pzParseN4pTableParts_(segment, headers[h].tag, headers[h].supplier);
      if (segParts && segParts.length) {
        for (var s = 0; s < segParts.length; s++) all.push(segParts[s]);
      }
    }
    if (all.length) return pzDedupeParts_(all);
  }
...
function getPackzettelPartsForStock(stockId) {
    ...
    return withRefurbDocumentLock_(function() {
      ...
      for (var i = 1; i < data.length; i++) {
        if (cellMatchesStockId(data[i][0], stockId)) {
          ...
          var verlaufEntry = logRegalVerlauf_(stockId, sheetRegal, regal, "speichern+regal");
          return {
            success: true,
  ...
  function assignRegalByScan(stockId, regal, action) {
    stockId = normalizeStockId(stockId);
    ...
    if (ok) {
      ...
      if (already) msg = stockId + " bereits in " + regalNorm;
    ...
    return {
      ...
      regal: regalNorm,
      sources: sources,
      previousRegal: prevRegal,
      already: already
    };
  }

  function revertRegalByScan(stockId, previousRegal) {
    stockId = normalizeStockId(stockId);
    if (!stockId) return { success: false, message: "Keine Stock-ID" };
    var regalNorm = normalizeRegalKeyForCount(previousRegal);
    if (regalNorm) {
      var moved = assignRegalByScan(stockId, regalNorm, "regal-revert");
      if (moved) moved.reverted = true;
      return moved;
    }
    ...
    return {
      success: ok,
      reverted: true,
      message: ok ? (stockId + " zurückgesetzt (kein Regal)") : "Zurücksetzen fehlgeschlagen",
      stockId: stockId,
      regal: "",
      previousRegal: prev
    };
